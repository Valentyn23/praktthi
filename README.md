...
### 📋 Локальні логи (при запуску через скрипти)

Логи зберігаються в папці `logs/`:
- `logs/bot.log` - Telegram бот
- `logs/webapp.log` - Веб-додаток
...
- Зберігається до 5 backup файлів
- UTF-8 кодування для підтримки кирилиці
- Формат: `час - модуль - рівень - повідомлення`

### Supervisor логи (для production):
```bash
